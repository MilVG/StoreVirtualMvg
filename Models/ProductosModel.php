<?php 
    class ProductosModel extends Mysql{
		...
		public function updateProducto(int $idproducto, string $nombre, string $descripcion, int $codigo, int $categoriaid, string $precio, int $stock,string $ruta, int $status){
			...
			$this->intStatus = $status;
			$return = 0;
			$sql = "SELECT * FROM productos WHERE codigo = '{$this->intCodigo}' AND id_Producto != $this->intIdProducto ";
			$request = $this->select_all($sql);
			if(empty($request))
			{
		...
		public function deleteProducto(int $idproducto){
			$this->intIdProducto = $idproducto;
			$sql = "UPDATE productos SET status = ? WHERE id_Producto = $this->intIdProducto ";
			$arrData = array(0);
			$request = $this->update($sql,$arrData);
			return $request;
		}
